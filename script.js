...
function refreshLobbies() {
    ...
    // Firebase will populate this later.
}
...
function initializeGame() {
    ...
    const savedLobby =
        JSON.parse(localStorage.getItem("deadLastLobby") || "null");
...
function endTurn() {
    ...
    addLog(
        `${gameState.players[gameState.currentPlayer - 1].name}'s turn ended.`
    );
...
document.addEventListener(
    ...
    () => {
        ...
        // Generate a temporary lobby code
        const codeElement =
        ...
        if (codeElement) {
            ...
            const existingCode =
                localStorage.getItem("deadLastLobbyCode");
        ...
        // Load saved lobby settings
        const savedLobby =
            JSON.parse(
                localStorage.getItem("deadLastLobby")
                || "null"
            );
